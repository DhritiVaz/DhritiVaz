...
**B.Tech CS · VIT Chennai · 2024–Present**
...
<details open>
<summary><b>Student Dashboard & Management System</b></summary>
<br/>

> Unified academic workspace — VTOP + LMS data, all in one dark-themed interface.

`React.js` `TypeScript` `Node.js` `Express.js` `PostgreSQL` `Prisma ORM` `Tailwind CSS`

- Attendance tracker with bunk calculator
- CGPA/GPA system with hypothetical grade simulator
- LMS sync — assignments, quizzes, files, deadlines
- Notes (markdown), tasks, file manager, calendar, timetable
- JWT + Google OAuth authentication

[GitHub ↗](https://github.com/DhritiVaz/student-dashboard)
...
<details open>
<summary><b>Landroid — AI Land Monitoring System</b></summary>
<br/>

> Drone imagery → NDVI plant health maps. AI-powered land monitoring in Flutter.

`Flutter` `FastAPI` `Python` `Supabase` `PostgreSQL` `PostGIS` `GeoTIFF` `NDVI`

- Processes GeoTIFF orthomosaic drone imagery
- Generates NDVI-based plant health maps and insights
- Parcel management with consultant and landowner roles
- Supabase + PostGIS for spatial data and documents

[GitHub ↗](https://github.com/DhritiVaz/landroid)
...
> Natural language → structured finance. Deployed multi-user tracker with smart savings.

`Python` `Flask` `PostgreSQL` `SQLAlchemy` `JavaScript` `Chart.js`

- Natural language input: `"lunch 80 today"` just works
- Smart savings planner with category-level recommendations
- Personality-based insights and streak tracking
- 4-tab Chart.js dashboard — Overview, Expenses, Budget, Reports

[GitHub ↗](https://github.com/DhritiVaz/expense-tracker) · [Live ↗](https://expense-tracker-ev7t.onrender.com)
...
<details open>
<summary><b>Krypt — Secure Password Manager</b></summary>
<br/>

> Zero-knowledge architecture. The server never sees your key. Ever.

`Python` `Flask` `PostgreSQL` `AES-256-GCM` `Argon2id` `JWT` `TOTP` `Docker`

- Key derived at runtime from master password via Argon2id
- AES-256-GCM with unique IV per vault entry
- JWT with refresh rotation + TOTP 2FA + account lockout
- **96 tests · 93% coverage** — SQL injection, auth bypass, user isolation

[GitHub ↗](https://github.com/DhritiVaz/krypt)
...
<details open>
<summary><b>Centralised Billing & Inventory System</b></summary>
<br/>

> Role-based billing across admin, franchise, and cashier workflows.

`Node.js` `PostgreSQL`

- JSONB invoice schema for dynamic multi-product transactions
- Franchise-level inventory tracking and product management
- RESTful APIs for end-to-end sales tracking and reporting

[GitHub ↗](https://github.com/DhritiVaz/centralised-billing-system)
